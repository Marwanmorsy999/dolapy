...
function showStyle(){
  ...
  renderShell();
  $('#engineBody')?.scrollIntoView({behavior:'smooth',block:'start'});
}
...
function renderShell(){
  ...
  document.querySelectorAll('#engineBody .engine-v3-item-label').forEach(e=>covered.add(e.textContent));
  if($('#statItems'))$('#statItems').textContent=count;
  if($('#styleHero'))$('#styleHero').disabled=count<1;
  if($('#statCoverage'))$('#statCoverage').textContent=count?'—':'—';
...
document.addEventListener('click',e=>{
  ...
  if(filterButton){filter=filterButton.dataset.filter;renderWardrobe();return}
  const favButton=e.target.closest('[data-fav]');
  if(favButton){
    const id=favButton.dataset.fav;
    items=items.map(x=>x.id===id?{...x,favorite:!x.favorite}:x);
  ...
  if(removeButton){
    ...
    localStorage.setItem(STORE_KEY,JSON.stringify(items));
    document.dispatchEvent(new CustomEvent('dolapy:items-changed'));
    return;
  }
  if(e.target.id==='emptyUpload'||e.target.id==='wardrobeEmptyUpload')$('#addHeroAI')?.click();
  if(e.target.id==='styleHero')showStyle();
});
$('#navStyle')?.addEventListener('click',showStyle);
$('#navWardrobe')?.addEventListener('click',showWardrobe);
$('#bottomStyle')?.addEventListener('click',showStyle);
$('#bottomWardrobe')?.addEventListener('click',showWardrobe);
