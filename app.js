...
const COLOR_WORDS = {
  black:['black','charcoal','graphite'], white:['white','cream','ivory'], grey:['grey','gray'], neutral:['beige','tan','camel','khaki'],
  brown:['brown','chocolate'], blue:['navy','blue','denim','cobalt','teal'], green:['green','olive','sage'], red:['red','burgundy','maroon','wine'],
  orange:['orange','rust','terracotta'], yellow:['yellow','mustard','gold'], purple:['purple','lavender','lilac'], pink:['pink','rose']
};
const COLOR_SCORE = {
...
function loadItems(){ try{return JSON.parse(localStorage.getItem(STORE_KEY)||'[]');}catch{return[];} }
function persist(){
  try{localStorage.setItem(STORE_KEY,JSON.stringify(items));}
  catch{alert('Your browser storage is full. Remove an older photo and try again.');}
  renderAll();
}
...
function inferCategory(name){const s=name.toLowerCase();if(/dress|gown|maxi|midi/.test(s))return'dresses';if(/shoe|sneaker|trainer|boot|loafer|sandal|heel|slide/.test(s))return'shoes';if(/jacket|coat|blazer|bomber|parka|overshirt|cardigan|puffer|hoodie/.test(s))return'outerwear';if(/pants|trouser|jeans|denim|shorts|skirt|cargo|jogger|chino/.test(s))return'bottoms';return'tops';}
function inferStyle(name){const s=name.toLowerCase();if(/street|oversized|graphic|skater/.test(s))return'streetwear';if(/formal|tailored|blazer|office|dressy|oxford/.test(s))return'smart';if(/sport|gym|active|running/.test(s))return'athletic';if(/utility|workwear|military|cargo/.test(s))return'utility';if(/vintage|retro/.test(s))return'vintage';if(/minimal|clean|simple/.test(s))return'minimal';return'casual';}
function inferMeta(file){
  const name=file.name.replace(/\.[^/.]+$/,'').replace(/[-_]+/g,' ').replace(/\s+/g,' ').trim()||'Untitled item';
  const lower=name.toLowerCase(); const category=inferCategory(name); const style=inferStyle(name);
  ...
  return{id:uid(),name,image:'',category,color,style,season,occasion:occasionValue,silhouette,pattern,warmth:category==='outerwear'?4:category==='shoes'?2:3,formality:style==='smart'?4:style==='athletic'?1:2,wearCount:0,favorite:false,createdAt:Date.now()};
}
function compressImage(file){
  return new Promise((resolve,reject)=>{
    const reader=new FileReader(); reader.onerror=reject;
    reader.onload=()=>{const img=new Image();img.onload=()=>{const max=1200,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);resolve(c.toDataURL('image/jpeg',.82));};img.onerror=reject;img.src=reader.result;}; reader.readAsDataURL(file);
  });
}
async function startUpload(fileList){
  const files=[...(fileList||[])].filter(f=>f.type&&f.type.startsWith('image/')); if(!files.length)return;
  try{pendingQueue=[];for(const file of files){const meta=inferMeta(file);meta.image=await compressImage(file);pendingQueue.push(meta);}nextPending();}
...
function nextPending(){
  ...
  $('#previewImg').src=currentPending.image; $('#fName').value=currentPending.name; $('#fCategory').value=currentPending.category; $('#fColor').value=currentPending.color; $('#fStyle').value=currentPending.style;
  $('#queueInfo').textContent=pendingQueue.length?`${pendingQueue.length} more image${pendingQueue.length===1?'':'s'} queued`:'Last item in this upload'; $('#modal').hidden=false; setTimeout(()=>$('#fName').focus(),40);
}
function closeModal(){pendingQueue=[];currentPending=null;$('#modal').hidden=true;}
function saveCurrent(){
  if(!currentPending)return;
  currentPending.name=firstNonBlank($('#fName').value)||'Untitled item'; currentPending.category=$('#fCategory').value; currentPending.color=firstNonBlank($('#fColor').value); currentPending.style=$('#fStyle').value;
  items.unshift(currentPending); currentPending=null;
...
function pairColor(a,b){const x=colorFamily(a.color),y=colorFamily(b.color);if(x==='unknown'||y==='unknown')return.76;return COLOR_SCORE[x]?.[y]??COLOR_SCORE[y]?.[x]??.72;}
function styleCompatibility(list){
  if(list.length<2)return.72; const fams=list.map(x=>styleFamily(`${x.style} ${x.name}`));let total=0,pairs=0;
  for(let i=0;i<fams.length;i++)for(let j=i+1;j<fams.length;j++){pairs++;const set=new Set([fams[i],fams[j]]);if(fams[i]===fams[j])total+=1;else if(set.has('casual')&&set.has('minimal'))total+=.96;else if(set.has('street')&&set.has('utility'))total+=.94;else if(set.has('smart')&&set.has('preppy'))total+=.96;else if(set.has('casual')&&set.has('street'))total+=.9;else if(set.has('casual')&&set.has('athletic'))total+=.84;else total+=.52;}
  return total/pairs;
}
function silhouetteScore(top,bottom){
  if(!top||!bottom)return.92; const p=`${top.silhouette}:${bottom.silhouette}`;
...
function patternScore(list){const n=list.filter(x=>x.pattern!=='solid').length;return n<=1?1:n===2?.84:.62;}
...
function compatibility(list){
  let colors=0,pairs=0;for(let i=0;i<list.length;i++)for(let j=i+1;j<list.length;j++){colors+=pairColor(list[i],list[j]);pairs++;}
  const color=pairs?colors/pairs:.72; const f=list.map(x=>x.formality),avg=f.reduce((a,b)=>a+b,0)/f.length,variance=f.reduce((s,x)=>s+(x-avg)**2,0)/f.length,formalFit=Math.max(.35,1-Math.sqrt(variance)/2.2);
  return .35*color+.25*styleCompatibility(list)+.15*formalFit+.15*silhouetteScore(list.find(x=>x.category==='tops'),list.find(x=>x.category==='bottoms'))+.10*patternScore(list);
}
function validOutfit(list){
  const cats=list.map(x=>x.category); if(cats.includes('dresses'))return cats.every(x=>['dresses','shoes','outerwear','accessories'].includes(x))&&cats.filter(x=>x==='dresses').length===1;
  const tops=cats.filter(x=>x==='tops').length,bottoms=cats.filter(x=>x==='bottoms').length,shoes=cats.filter(x=>x==='shoes').length;return tops===1&&bottoms===1&&shoes<=1;
}
function candidateSets(){
  const tops=items.filter(x=>x.category==='tops').slice(0,16),bottoms=items.filter(x=>x.category==='bottoms').slice(0,16),dresses=items.filter(x=>x.category==='dresses').slice(0,10),shoes=items.filter(x=>x.category==='shoes').slice(0,10),outer=items.filter(x=>x.category==='outerwear').slice(0,8),acc=items.filter(x=>x.category==='accessories').slice(0,8),out=[];
  for(const d of dresses){out.push([d]);for(const s of shoes)out.push([d,s]);for(const o of outer.slice(0,2))out.push([d,o]);for(const s of shoes.slice(0,2))for(const o of outer.slice(0,2))out.push([d,s,o]);}
  for(const t of tops)for(const b of bottoms){out.push([t,b]);for(const s of shoes)out.push([t,b,s]);for(const o of outer.slice(0,2))out.push([t,b,o]);for(const s of shoes.slice(0,2))for(const o of outer.slice(0,2))out.push([t,b,s,o]);for(const a of acc.slice(0,2))out.push([t,b,a]);}
  return out.filter(validOutfit);
}
function outfitScore(set,used){
  const freshness=set.reduce((s,x)=>s+1/(1+(x.wearCount||0)),0)/set.length,novel=set.reduce((s,x)=>s+(used.has(x.id)?0:1),0)/set.length,favorite=set.reduce((s,x)=>s+(x.favorite?.08:0),0)/set.length;
  return 100*(.70*compatibility(set)+.15*contextScore(set)+.08*freshness+.05*Math.min(1,novel)+favorite);
}
function outfitName(set){if(set.some(x=>x.category==='dresses'))return'The clean dress edit';const f=set.map(x=>styleFamily(`${x.style} ${x.name}`));if(f.includes('street'))return'The everyday street edit';if(f.includes('smart'))return'The sharp everyday edit';if(f.includes('utility'))return'The utility mix';return'The easy everyday edit';}
function explainOutfit(set){const colors=set.map(x=>x.color).filter(Boolean),tone=colors.length>=2?'a balanced color mix':'a simple neutral base',styles=[...new Set(set.map(x=>styleFamily(`${x.style} ${x.name}`)))];return`Built around ${set[0].name} with ${tone}, ${styles.join(' + ')} styling, and a ${occasion} context.`;}
function buildOutfits(){
  const pool=candidateSets(),selected=[],used=new Set();
  for(let round=0;round<Math.min(8,Math.max(1,Math.ceil(items.length/2)))&&pool.length;round++){
    pool.sort((a,b)=>outfitScore(b,used)-outfitScore(a,used)); const best=pool.shift(); const score=Math.max(55,Math.min(99,Math.round(outfitScore(best,used))));
    selected.push({id:best.map(x=>x.id).sort().join('-'),items:best,score,name:outfitName(best),usedItemIds:best.map(x=>x.id),explanation:explainOutfit(best)}); best.forEach(x=>used.add(x.id));
    for(let i=pool.length-1;i>=0;i--){const overlap=pool[i].filter(x=>best.some(y=>y.id===x.id)).length;if(overlap>=Math.min(best.length,2))pool.splice(i,1);}
  }
  outfits=selected;activeOutfitIndex=0;items.forEach(item=>{if(used.has(item.id))item.wearCount=(item.wearCount||0)+1;});
  try{localStorage.setItem(STORE_KEY,JSON.stringify(items));}catch{} renderStyle();
}
...
function renderStyle(){
  const used=new Set(outfits.flatMap(o=>o.usedItemIds)); $('#statItems').textContent=items.length; $('#statCoverage').textContent=outfits.length&&items.length?`${Math.round(used.size/items.length*100)}%`:'—'; $('#statScore').textContent=outfits[0]?`${outfits[0].score}%`:'—'; $('#styleHero').disabled=items.length<2;
  const body=$('#engineBody');
  if(!items.length){body.innerHTML=`<div class="empty"><div>＋</div><h3>Start with your real wardrobe</h3><p>Upload a few clothing photos. Dolapy stores them locally in this browser and uses a deterministic styling engine to build looks.</p><button class="primary" id="emptyUpload">Upload a piece</button></div>`;$('#alts').hidden=true;return;}
  ...
  const alts=outfits.filter((_,i)=>i!==activeOutfitIndex).slice(0,3);$('#alts').hidden=!alts.length;$('#altGrid').innerHTML=alts.map(o=>cardHtml(o,outfits.indexOf(o))).join('');
}
...
function renderWardrobe(){
  renderFilters(); const shown=filter==='all'?items:items.filter(x=>x.category===filter);
